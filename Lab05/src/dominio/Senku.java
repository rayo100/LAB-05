...
public class Senku {
    ...
    private final int llena = 1;
    private final int imposible = 2;
    ...
    public void inicializarMatriz(JButton[][] matrizButton) {
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                ...
                } else if (matrizButton[i][j].getBackground().equals(Color.GREEN)) {
                    matrizPiezas[i][j] = llena;
                } else {
                    matrizPiezas[i][j] = imposible;
                }
            }
        }
    }
    public boolean movePieza(int pos1x, int pos1y, int pos2x, int pos2y) throws SenkuException {
        ...
        if (move && matrizPiezas[pos1x][pos1y] == 1) {
            matrizPiezas[pos2x][pos2y] = llena;
    ...
    public boolean moveNorte(int pos1x, int pos1y, int pos2x, int pos2y) {
        ...
        if ((pos1x - 2 == pos2x) && (pos1y == pos2y)) {
            if ((matrizPiezas[pos1x - 1][pos1y] == llena) && (matrizPiezas[pos2x][pos2y] == vacia)) {
                move = true;
    ...
    public boolean moveEste(int pos1x, int pos1y, int pos2x, int pos2y) {
        ...
        if ((pos1x == pos2x) && (pos1y + 2 == pos2y)) {
            if ((matrizPiezas[pos1x][pos1y + 1] == llena) && (matrizPiezas[pos2x][pos2y] == vacia)) {
                move = true;
